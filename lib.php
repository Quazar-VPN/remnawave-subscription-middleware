<?php

if (!defined('SUBMW')) define('SUBMW', 1);

require_once __DIR__ . '/lib/db.php';
require_once __DIR__ . '/lib/settings.php';
require_once __DIR__ . '/lib/overrides.php';
require_once __DIR__ . '/lib/substitution.php';
require_once __DIR__ . '/lib/logging.php';
require_once __DIR__ . '/lib/metrics.php';
require_once __DIR__ . '/lib/api.php';
require_once __DIR__ . '/lib/branding.php';
require_once __DIR__ . '/lib/grace.php';
require_once __DIR__ . '/lib/squadconf.php';
require_once __DIR__ . '/lib/wglease.php';
require_once __DIR__ . '/lib/vless.php';
require_once __DIR__ . '/lib/proto/trojan.php';
require_once __DIR__ . '/lib/proto/shadowsocks.php';
require_once __DIR__ . '/lib/proto/hysteria2.php';
require_once __DIR__ . '/lib/proto/tuic.php';
require_once __DIR__ . '/lib/security.php';
require_once __DIR__ . '/lib/rules.php';
require_once __DIR__ . '/lib/update.php';
require_once __DIR__ . '/lib/chat.php';
require_once __DIR__ . '/lib/landing.php';
require_once __DIR__ . '/lib/subpage.php';
require_once __DIR__ . '/lib/addsub.php';
require_once __DIR__ . '/lib/junk.php';
require_once __DIR__ . '/lib/chan.php';
require_once __DIR__ . '/lib/chanmw.php';
require_once __DIR__ . '/lib/clientver.php';
require_once __DIR__ . '/lib/gc.php';
